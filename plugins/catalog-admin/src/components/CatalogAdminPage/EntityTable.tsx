...
function mapEntityToTableRow(kind: string, idx: number, entity: Entity): TableRow {
    if (kind === 'api') {
        return {
            id: idx,
...
const getData = async (
    ...
    let items = result.items.map((entity, idx) => mapEntityToTableRow(kind, idx, entity));
...
export const EntityTable = ({ kind, system }: EntityTableProps) => {
    ...
    const fetchData = async (kind: string, offset: number, pageSize: number, sort: SortDescriptor | null) => {
    ...
    const { tableProps, reload } = useTable({
        mode: 'offset',
        getData: ({ offset, pageSize, sort }) => fetchData(kind, offset, pageSize, sort),
        paginationOptions: {
            pageSize: 5,
    ...
    const getSelectedEntityNames = () => {
        if (selected === 'all') {
            ...
            return tableProps.data?.filter(row => selected.has(row.id.toString())).map(row => row.entityName) || [];
        }
    }
    ...
    const deleteSelected = async () => {
        ...
        for (const entityName of entityNames) {
            try {
                ...
                toastApi.post({ title: `Successfully deleted entity: ${entityName}`, status: 'success', timeout: 1500 });
            } catch (error) {
                toastApi.post({ title: `Failed to delete entity: ${entityName} - ${error}`, status: 'danger', timeout: 1500 });
    ...
    useEffect(() => {
        ...
        if (prevKindSystem.current.kind !== kind || prevKindSystem.current.system !== system) {
                    ...
                    <>
                        <DialogTrigger>
                            ...
                            <Dialog>
                                ...
                                <DialogBody>
                                    <Text>Are you sure you want to delete {selected === 'all' ? 'all items' : `${selected.size} items`}?</Text>
                                ...
                                <DialogFooter>
                                    ...
                                    <Button destructive variant="primary" slot="close" onClick={deleteSelected}>Delete</Button>
                                </DialogFooter>
                            </Dialog>
                        </DialogTrigger>
                    </>
