...
export const DrawIoAddOn = ({
  ...
  const observer = new MutationObserver(() => {
    if (!enableEdit)
      document.querySelectorAll('[title="Edit"]').forEach(el => el.remove());
    if (!enablePrint)
      document.querySelectorAll('[title="Print"]').forEach(el => el.remove());
  });
      ...
      if ((window as any).GraphViewer) {
        ...
        divs.forEach((div, _) => {
          if (div.hasAttribute('data-mxgraph')) {
            (window as any).GraphViewer.createViewerForElement(
              div,
              (viewer: any) => {
