...
const getServiceInfo = (entity: Entity): EntityInfo | null => {
  ...
  const serviceVersion =
    entity.metadata.annotations?.[ANNOTATION_SERVICE_VERSION];
  ...
  const imageVersion =
    entity.metadata.annotations?.[ANNOTATION_IMAGE_VERSION] ?? 'n/a';
  const platform =
    entity.metadata.annotations?.[ANNOTATION_SERVICE_PLATFORM] ??
    DEFAULT_PLATFORM;
