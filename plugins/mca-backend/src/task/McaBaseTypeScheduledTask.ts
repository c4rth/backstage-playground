...
export class McaBaseTypeScheduledTask {
  ...
  async runAsync() {
    const listBaseTypesBaseUrl = this.config.getString(
      'mcaComponents.baseTypes.listBaseUrl',
    );
    this.logger.info(
      `Scheduled task: get basetypes from ${listBaseTypesBaseUrl}`,
    );
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch BaseTypes HTML: ${response.statusText}`,
        );
      }
