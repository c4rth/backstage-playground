...
export class McaOperationScheduledTask {
  ...
  async runAsync() {
    const allOperationsCsvBaseUrl = this.config.getString(
      'mcaComponents.operations.csvBaseUrl',
    );
    this.logger.info(
      `Scheduled task: get all operations CSV from ${allOperationsCsvBaseUrl}`,
    );
    try {
        ...
        Readable.from(csvData)
          ...
          .on('data', data => {
            try {
              ...
              if (
                !data[columns.component].startsWith('Operation') &&
                !data[columns.component].startsWith('Element')
              )
                return;
              components.push({
          ...
          .on('end', async () => {
            try {
              ...
              if (mcaVersions) {
                promises.push(
                  this.mcaComponentsStore.addOrUpdateMcaVersions(mcaVersions),
                );
              }
              ...
              this.logger.info(
                `CSV data parsed successfully. Processed ${components.length} components`,
              );
              resolve();
            } catch (error) {
