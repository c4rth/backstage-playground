...
export const ApiDefinitionSwaggerCard = () => {
...
export const ApiDefinitionRawCard = () => {
  const { entity } = useEntity<ApiEntity>();
  const definition = entity.spec.definition.toString();

  return <CodeSnippet text={definition} language="yaml" showCopyCodeButton />;
};

export const ApiDefinitionServicesCard = () => {
  const { entity } = useEntity<ApiEntity>();
  const apiVersion =
    entity.metadata.annotations?.[ANNOTATION_API_VERSION]?.toString();

  return (
    <Tabs key="tabs-services" defaultSelectedKey="tab-api-version">
      <TabList>
        <Tab id="tab-api-version">{apiVersion}</Tab>
        <Tab id="tab-api-all-versions">All versions</Tab>
      </TabList>
      <TabPanel id="tab-api-version">
        <Flex direction="column" gap="4">
          <ApiRelationCard dependency="provider" />
          <ApiRelationCard dependency="consumer" />
        </Flex>
      </TabPanel>
      <TabPanel id="tab-api-all-versions">
        <Flex direction="column" gap="4">
...
export const ApiDefinitionInfoCard = () => {
  const { entity } = useEntity<ApiEntity>();
