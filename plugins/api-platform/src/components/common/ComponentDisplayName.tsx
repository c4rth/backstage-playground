...
export const ComponentDisplayName = ({ text, type }: ComponentDisplayNameProps): React.JSX.Element => {
    return (
        <Flex gap='xs' style={{ alignItems: 'center', textDecoration: 'inherit', color: 'inherit' }}>
            {type === 'api' && (
                <Box style={ICON_WRAPPER_STYLE}><RiPuzzleFill style={ICON_STYLE} /></Box>
            )}
            {type === 'system' && (
                <Box style={ICON_WRAPPER_STYLE}><RiShapesFill style={ICON_STYLE} /></Box>
            )}
            {type === 'service' && (
                <Box style={ICON_WRAPPER_STYLE}><RiCpuLine style={ICON_STYLE} /></Box>
            )}
            {type === 'azdo' && (
                <Box style={ICON_WRAPPER_STYLE}><AzureDevOpsIcon style={ICON_STYLE} /></Box>
            )}
            {type === 'url' && (
                <Box style={ICON_WRAPPER_STYLE}><RiGlobalLine style={ICON_STYLE} /></Box>
            )}
            {type === 'library' && (
                <Box style={ICON_WRAPPER_STYLE}><RiBookShelfLine style={ICON_STYLE} /></Box>
            )}
            {text}
        </Flex>
    );
