...
export class McaComponentsBackendClient implements McaComponentsBackendApi {
  ...
  async getMcaComponentsCount(type: McaComponentType): Promise<number> {
    const url = new URL(
      `${await this.discoveryApi.getBaseUrl('mca')}/mca/count?type=${type}`
    );
  ...
  async getMcaComponent(component: string): Promise<McaComponent | undefined> {
    const url = new URL(
      `${await this.discoveryApi.getBaseUrl('mca')}/mca/components/${component}`
    );
    const response = await this.fetchApi.fetch(url);
    const item = await response.json();
    return (
  ...
  async getMcaVersions(): Promise<McaVersions> {
    const url = new URL(
      `${await this.discoveryApi.getBaseUrl('mca')}/mca/versions`
    );
    const response = await this.fetchApi.fetch(url);
    const item = await response.json();
    return (
  ...
  async getMcaBaseTypesCount(): Promise<number> {
    const url = new URL(
      `${await this.discoveryApi.getBaseUrl('mca')}/basetypes/count`
    );
    const response = await this.fetchApi.fetch(url);
    const item = await response.json();
    return (
  ...
  async getMcaBaseType(baseType: string): Promise<McaBaseType | undefined> {
    const url = new URL(
      `${await this.discoveryApi.getBaseUrl('mca')}/baseTypes/components/${baseType}`
    );
    const response = await this.fetchApi.fetch(url);
    const item = await response.json();
    return (
