...
import { apiPlatformPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [apiPlatformPlugin],
    });

    await request(server).get('/api/api-platform/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/api-platform/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/api-platform/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/api-platform/todos/${createdTodoItem.id}`)
