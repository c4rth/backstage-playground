...
export class PBEStringDecryptor {
  ...
  public encrypt(plainText: string, password: string | null): string {
    ...
    const cipherText = encrypted.ciphertext || WordArray.create([], 0); // Ensure cipherText is a WordArray
  ...
  private deriveKeyAndIV(password: string, salt: WordArray): { key: WordArray; iv: WordArray } {
    ...
    for (let i = 1; i < PBEStringDecryptor.ITERATIONS; i++) {
      md5.reset();
      md5.update(hash);
      hash = md5.finalize();
    }
