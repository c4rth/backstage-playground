...
export async function createRouter(
  ...
  router.get('/dashboard-pull-requests/:projectName', async (req, res) => {
    ...
    if (host || org) {
      if (!host || !org) {
        throw new InputError(
          'Both host and org query parameters are required when either is provided',
        );
      }
  ...
  /* Custom routes for scaffolder extensions */

  router.get('/projects/:projectName/repositories', async (req, res) => {
    const { projectName } = req.params;
    const host = req.query.host?.toString();
    const org = req.query.org?.toString();
  ...
  router.get(
    '/projects/:projectName/repositories/:repoName/branches',
    async (req, res) => {
      const { projectName, repoName } = req.params;
      const host = req.query.host?.toString();
      const org = req.query.org?.toString();

      const branches = await azureDevOpsApi.getBranches(
        projectName,
        repoName,
        host,
        org,
  ...
  /* End of custom routes for scaffolder extensions */
