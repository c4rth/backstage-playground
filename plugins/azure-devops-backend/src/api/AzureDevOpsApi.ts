...
export class AzureDevOpsApi {
  ...
  /* Custom methods for scaffolder extensions */
  ...
  public async getBranches(
    ...
    const branches: GitBranchStats[] = await client.getBranches(
      repoId!,
      projectName,
    );
    const branchNames: string[] = branches
      ? branches.map(branch => branch?.name ?? '')
      : [];
    const branchNamesFiltered: string[] = branchNames.filter(
      branch => branch !== '',
    );

    return branchNamesFiltered.sort((a, b) => a.localeCompare(b));
  }

  /* End of custom methods for scaffolder extensions */
}
