  ...
  return {
    async groupTransformer(
      ...
      if (name.startsWith('TEAM') && !name.endsWith('.R')) {
        ...
        logger.debug(JSON.stringify(backstageGroup), {
          task: 'groupTransformer',
        });
        if (backstageGroup) {
    ...
    async organizationTransformer(
      graphOrganization: MicrosoftGraph.Organization,
    ): Promise<GroupEntity | undefined> {
      const backstageOrg =
        await defaultOrganizationTransformer(graphOrganization);
      logger.debug(JSON.stringify(backstageOrg), {
        task: 'organizationTransformer',
      });
      return backstageOrg;
    },
  };
